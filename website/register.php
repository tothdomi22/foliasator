...
if (isset($error_messagereg)) {
    header("Location: register_form.php?error_messagereg=" . urlencode($error_messagereg));
    exit;
}
...
FROM users WHERE username = '$user' or email = '$email'"; // Select username from users where username is $user.
